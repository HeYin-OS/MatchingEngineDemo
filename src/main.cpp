...
int main() {
    ...
    printTrades(engine.processOrder(Order(1, "AAPL", 150, 100, OrderSide::Sell, OrderType::Limit)));

    std::cout << "Order 2: Buy 10 AAPL @ 150 (Taker)" << std::endl;
    printTrades(engine.processOrder(Order(2, "AAPL", 150, 10, OrderSide::Buy, OrderType::Limit)));
    ...
    printTrades(engine.processOrder(Order(3, "AAPL", 160, 50, OrderSide::Sell, OrderType::Limit)));

    std::cout << "Order 4: Buy 50 AAPL @ 140" << std::endl;
    printTrades(engine.processOrder(Order(4, "AAPL", 140, 50, OrderSide::Buy, OrderType::Limit)));
    ...
    engine.processOrder(Order(10, "TSLA", 100, 10, OrderSide::Sell, OrderType::Limit));
    engine.processOrder(Order(11, "TSLA", 101, 20, OrderSide::Sell, OrderType::Limit));
    engine.processOrder(Order(12, "TSLA", 102, 50, OrderSide::Sell, OrderType::Limit));
    std::cout << "Setup: TSLA Sell Orders placed: 10@100, 20@101, 50@102" << std::endl;

    std::cout << "Order 13: BIG BUY 40 TSLA @ 105" << std::endl;
    auto tradesSweep = engine.processOrder(Order(13, "TSLA", 105, 40, OrderSide::Buy, OrderType::Limit));
    ...
    engine.processOrder(Order(20, "MSFT", 200, 10, OrderSide::Sell, OrderType::Limit));

    std::cout << "Order 21: Buy AAPL @ 200 (Should NOT match MSFT)" << std::endl;
    auto tradesIsolation = engine.processOrder(Order(21, "AAPL", 200, 10, OrderSide::Buy, OrderType::Limit));
